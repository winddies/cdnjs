...
class Canvas_Canvas {
  ...
  drawLinkedLine(p1, p2, opacity) {
    ...
    if (container.particles.lineLinkedColor === Constants.randomColorValue) {
      ...
      colorLine = {
        b: Math.floor(Utils_Utils.mix(sourceColor.b, destColor.b, p1.size.value, p2.size.value)),
        g: Math.floor(Utils_Utils.mix(sourceColor.g, destColor.g, p1.size.value, p2.size.value)),
        r: Math.floor(Utils_Utils.mix(sourceColor.r, destColor.r, p1.size.value, p2.size.value))
      };
    } else {
  ...
  drawParticle(particle) {
    ...
    const opacity = (_b = particle.bubble.opacity) !== null && _b !== void 0 ? _b : particle.opacity.value;
    const color = (_c = particle.bubble.color) !== null && _c !== void 0 ? _c : particle.color;

    if (color) {
      colorValue = ColorUtils_ColorUtils.getStyleFromColor(color, opacity);
    }
...
class Repulser_Repulser {
  ...
  divRepulse() {
    ...
    const velocity = 100;
    const repulseFactor = Utils_Utils.clamp((1 - Math.pow(distDiv / repulseRadius, 2)) * velocity, 0, 50);
    const newPos = {
    ...
    if (outMode === OutMode.bounce || outMode === OutMode.bounceVertical || outMode === OutMode.bounceHorizontal) {
      const isInside = {
        horizontal: newPos.x - particle.size.value > 0 && newPos.x + particle.size.value < container.canvas.dimension.width,
        vertical: newPos.y - particle.size.value > 0 && newPos.y + particle.size.value < container.canvas.dimension.height
      };
  ...
  hoverRepulse() {
    ...
    if (outMode === OutMode.bounce || outMode === OutMode.bounceVertical || outMode === OutMode.bounceHorizontal) {
      const isInside = {
        horizontal: pos.x - particle.size.value > 0 && pos.x + particle.size.value < container.canvas.dimension.width,
        vertical: pos.y - particle.size.value > 0 && pos.y + particle.size.value < container.canvas.dimension.height
      };
  ...
  processRepulse(dx, dy, force) {
    ...
    if (outMode === OutMode.bounce || outMode === OutMode.bounceHorizontal || outMode === OutMode.bounceVertical) {
      ...
      if (outMode !== OutMode.bounceVertical) {
        if (pos.x + particle.size.value > container.canvas.dimension.width || pos.x - particle.size.value < 0) {
          particle.velocity.horizontal = -particle.velocity.horizontal;
        }
      }

      if (outMode !== OutMode.bounceHorizontal) {
        if (pos.y + particle.size.value > container.canvas.dimension.height || pos.y - particle.size.value < 0) {
...
class Updater_Updater {
  ...
  fixOutOfCanvasPosition() {
    ...
    if (outMode === OutMode.destroy) {
      ...
      if (!Utils_Utils.isPointInside(particle.position, container.canvas.dimension, sizeValue)) {
  ...
  updateBounce() {
    ...
    if (options.polygon.enable && options.polygon.type !== PolygonMaskType.none && options.polygon.type !== PolygonMaskType.inline) {
      ...
      if (outMode === OutMode.bounce || outMode === OutMode.bounceHorizontal) {
        Updater_Updater.checkBounds(x, particle.size.value, container.canvas.dimension.width, () => {
          particle.velocity.horizontal = -particle.velocity.horizontal;
        });
      }

      if (outMode === OutMode.bounce || outMode === OutMode.bounceVertical) {
        Updater_Updater.checkBounds(y, particle.size.value, container.canvas.dimension.height, () => {
          particle.velocity.vertical = -particle.velocity.vertical;
        });
      }
    }
...
class Particle_Particle {
  ...
  update(index, delta) {
    ...
    if (Utils_Utils.isInArray(HoverMode.grab, hoverMode)) {
      Grabber_Grabber.grab(this, container);
    }
...
class Emitter_Emitter {
  ...
  emit() {
    ...
    const particle = new Particle_Particle(container, position, this);
...
class EventListeners_EventListeners {
  ...
  static manageListener(element, event, handler, add, options) {
    if (add) {
      EventListeners_EventListeners.addListener(element, event, handler, options);
    } else {
    ...
    if (this.canPush) {
      ...
      switch (options.interactivity.events.onClick.mode) {
        ...
        case ClickMode.emitter:
          ...
          const emitter = new Emitter_Emitter(container, emitterOptions, position);
...
class SpatialGrid_SpatialGrid {
  ...
  queryInCell(position) {
    ...
    if (Array.isArray(this.grid[pos.x]) && Array.isArray(this.grid[pos.x][pos.y])) return this.grid[pos.x][pos.y] || [];else return [];
  }
  ...
  queryRadius(position, radius) {
    ...
    for (let i = 0; i < items.length; i++) {
      if (items[i] && Utils_Utils.getDistanceBetweenCoordinates(items[i].position, position) <= radius) out[out.length + 1] = items[i];
    }
  ...
  queryRadiusWithDist(position, radius) {
    ...
    for (let i = 0; i < items.length; i++) {
      if (items[i]) {
        const dist = Utils_Utils.getDistanceBetweenCoordinates(items[i].position, position);
        if (dist <= radius) out.push({
          dist: dist,
          particle: items[i]
        });
      }
  ...
  select(start, end) {
    ...
    for (let x = start.x; x <= end.x; x++) {
      ...
      for (let y = start.y; y <= end.y; y++) {
        ...
        for (let i = 0; i < this.grid[x][y].length; i++) if (this.grid[x][y][i] != null) out[out.length + 1] = this.grid[x][y][i];
      }
    }
...
class Classes_Particles_Particles {
  constructor(container) {
    ...
    this.spatialGrid = new SpatialGrid_SpatialGrid(this.container.canvas.dimension);
  }
  ...
  update(delta) {
    for (let i = 0; i < this.array.length; i++) {
      ...
      if (this.interactionsEnabled) {
        InteractionManager_InteractionManager.interact(p, this.container);
      }
    }
  }
