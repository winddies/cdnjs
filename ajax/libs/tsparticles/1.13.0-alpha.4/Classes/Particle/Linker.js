...
var Linker = (function () {
    ...
    Linker.link = function (p1, container) {
        ...
        for (var _i = 0, _b = container.particles.spatialGrid.queryRadiusWithDist(p1.position, optDistance); _i < _b.length; _i++) {
            var p2 = _b[_i];
            if (p1 === p2.particle || !p2.particle.particlesOptions.lineLinked.enable)
                continue;
            var opacityLine = optOpacity - (p2.dist * optOpacity) / optDistance;
            if (opacityLine > 0) {
                if (!container.particles.lineLinkedColor) {
                    var color = p1.particlesOptions.lineLinked.color;
