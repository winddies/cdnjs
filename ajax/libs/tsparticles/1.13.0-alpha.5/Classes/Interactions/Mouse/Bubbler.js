...
var Bubbler = (function () {
    ...
    Bubbler.clickBubble = function (container) {
        ...
        for (var _i = 0, _b = container.particles.spatialGrid.queryRadius(mouseClickPos, container.retina.bubbleModeDistance); _i < _b.length; _i++) {
            var particle = _b[_i];
    ...
    Bubbler.hoverBubble = function (container) {
        ...
        for (var _i = 0, _a = container.particles.spatialGrid.queryRadiusWithDistance(mousePos, container.retina.bubbleModeDistance); _i < _a.length; _i++) {
            var _b = _a[_i], distance = _b.distance, particle = _b.particle;
            if ((particle === null || particle === void 0 ? void 0 : particle.position) === undefined) {
                continue;
            }
            var ratio = 1 - distance / container.retina.bubbleModeDistance;
    ...
    Bubbler.hoverBubbleSize = function (container, particle, ratio) {
        ...
        if (modeSize > optSize) {
            var size = pSize + (modeSize - optSize) * ratio;
            particle.bubble.radius = Utils_1.Utils.clamp(size, pSize, modeSize);
        }
        else if (modeSize < optSize) {
            var size = pSize - (optSize - modeSize) * ratio;
            particle.bubble.radius = Utils_1.Utils.clamp(size, modeSize, pSize);
        }
    };
    Bubbler.hoverBubbleOpacity = function (container, particle, ratio) {
        ...
        if (modeOpacity > optOpacity) {
            var opacity = pOpacity + (modeOpacity - optOpacity) * ratio;
            particle.bubble.opacity = Utils_1.Utils.clamp(opacity, pOpacity, modeOpacity);
        }
        else if (modeOpacity < optOpacity) {
            var opacity = pOpacity - (optOpacity - modeOpacity) * ratio;
            particle.bubble.opacity = Utils_1.Utils.clamp(opacity, modeOpacity, pOpacity);
        }
    };
