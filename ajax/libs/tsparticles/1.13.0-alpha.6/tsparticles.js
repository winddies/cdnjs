...
class SpatialGrid_SpatialGrid {
  ...
  queryRadiusWithDistance(position, radius) {
    ...
    for (let i = 0; i < items.length; i++) {
      if (items[i]) {
        const distance = Utils_Utils.getDistanceBetweenCoordinates(items[i].position, position);
        if (distance <= radius) out.push({
          distance: distance,
          particle: items[i]
        });
      }
  ...
  select(start, end) {
    ...
    for (let x = start.x; x <= end.x; x++) {
      ...
      for (let y = start.y; y <= end.y; y++) {
        ...
        for (let i = 0; i < this.grid[x][y].length; i++) if (this.grid[x][y][i]) out[out.length + 1] = this.grid[x][y][i];
      }
    }
  ...
  static grab(container) {
    ...
    if (interactivity.events.onHover.enable && container.interactivity.status === Constants.mouseMoveEvent) {
      ...
      for (const {
        distance,
        particle
      } of container.particles.spatialGrid.queryRadiusWithDistance(mousePos, container.retina.grabModeDistance)) {
...
class Bubbler_Bubbler {
  ...
  static hoverBubble(container) {
    ...
    for (const {
      distance,
      particle
    } of container.particles.spatialGrid.queryRadiusWithDistance(mousePos, container.retina.bubbleModeDistance)) {
      if ((particle === null || particle === void 0 ? void 0 : particle.position) === undefined) {
        continue;
      }
