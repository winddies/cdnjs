...
class Updater_Updater {
  ...
  fixOutOfCanvasPosition() {
    ...
    if (outMode === OutMode.bounce) {
      newPos = {
        bottom: canvasSize.height - particle.size.value,
        left: particle.size.value * 2,
        right: canvasSize.width - particle.size.value,
        top: particle.size.value * 2
      };
...
class SpatialGrid_SpatialGrid {
  ...
  queryRadius(position, radius) {
    ...
    for (let i = 0; i < items.length; i++) {
      if (items[i] && Utils_Utils.getDistanceBetweenCoordinates(items[i].position, position) <= radius) {
