...
class CanvasUtils_CanvasUtils {
  ...
  static drawBlackHole(context, blackHole) {
    context.save();
    context.translate(blackHole.position.x, blackHole.position.y);
    context.beginPath();
    context.arc(0, 0, blackHole.size, 0, Math.PI * 2, false);
    context.closePath();
    context.fillStyle = ColorUtils_ColorUtils.getStyleFromColor(blackHole.color);
    context.fill();
    context.restore();
  }
...
class Canvas_Canvas {
  ...
  drawBlackHole(blackHole) {
    ...
    CanvasUtils_CanvasUtils.drawBlackHole(ctx, blackHole);
  }
  ...
  updateBounce() {
    ...
    if (options.polygon.enable && options.polygon.type !== PolygonMaskType.none && options.polygon.type !== PolygonMaskType.inline) {
      ...
      if (outMode === OutMode.bounce || outMode === OutMode.bounceHorizontal) {
        ...
        Updater_Updater.checkBounds(x, size, container.canvas.dimension.width, velocity, () => {
      ...
      if (outMode === OutMode.bounce || outMode === OutMode.bounceVertical) {
        ...
        Updater_Updater.checkBounds(y, size, container.canvas.dimension.height, velocity, () => {
...
// CONCATENATED MODULE: ./dist/Classes/BlackHole.js

class BlackHole_BlackHole {
  constructor(container, options, position) {
    ...
    const offset = (_b = options.position) !== null && _b !== void 0 ? _b : {
      x: 50,
      y: 50
    };
    ...
    if (this.canPush) {
      ...
      switch (options.interactivity.events.onClick.mode) {
        ...
        case ClickMode.blackHole:
          let blackHolesModeOptions;
          const modeBlackHoles = options.interactivity.modes.blackHoles;

          if (modeBlackHoles instanceof Array) {
            if (modeBlackHoles.length > 0) {
              blackHolesModeOptions = Utils_Utils.itemFromArray(modeBlackHoles);
            }
          } else {
            blackHolesModeOptions = modeBlackHoles;
          }

          const blackHolesOptions = blackHolesModeOptions !== null && blackHolesModeOptions !== void 0 ? blackHolesModeOptions : options.blackHoles instanceof Array ? Utils_Utils.itemFromArray(options.blackHoles) : options.blackHoles;
          const bhPosition = container.interactivity.mouse.clickPosition;
          const blackHole = new BlackHole_BlackHole(container, blackHolesOptions, bhPosition);
...
// CONCATENATED MODULE: ./dist/Classes/Options/BlackHoles/BlackHoleRandomSize.js
class BlackHoleRandomSize {
  constructor() {
    this.enable = false;
    this.minimumValue = 1;
  }
...
// CONCATENATED MODULE: ./dist/Classes/Options/BlackHoles/BlackHoleSize.js

class BlackHoleSize_BlackHoleSize {
  constructor() {
    this.value = 50;
...
// CONCATENATED MODULE: ./dist/Classes/Options/BlackHoles/BlackHole.js


class BlackHoles_BlackHole_BlackHole {
  constructor() {
    this.color = new OptionsColor();
    this.color.value = "#000000";
    this.size = new BlackHoleSize_BlackHoleSize();
  }
      ...
      if (data.blackHoles !== undefined) {
        if (data.blackHoles instanceof Array) {
          this.blackHoles = data.blackHoles.map(s => {
            const tmp = new BlackHoles_BlackHole_BlackHole();
            tmp.load(s);
            return tmp;
          });
        } else {
          if (this.blackHoles instanceof Array) {
            this.blackHoles = new BlackHoles_BlackHole_BlackHole();
          }
  ...
  init() {
    ...
    if (this.options.blackHoles instanceof Array) {
      for (const blackHoleOptions of this.options.blackHoles) {
        const blackHole = new BlackHole_BlackHole(this, blackHoleOptions);
